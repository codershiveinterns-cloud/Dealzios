...
export default function CouponsDirectoryPage() {
  ...
  const filteredCoupons = COUPONS.filter(c => {
    ...
    return c.categorySlug === selectedCategory;
  });
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        <div className="bg-white rounded-3xl border border-slate-200/90 p-8 shadow-xs mb-8 space-y-4">
          <div className="flex items-center gap-2 text-indigo-600 font-extrabold text-xs uppercase tracking-wider">
            <Tag className="w-4 h-4" /> Verified Promo Codes
          </div>
          <h1 className="text-3xl sm:text-4xl font-black text-slate-950 tracking-tight">
            Latest Working Coupon Codes
          </h1>
          <p className="text-sm text-slate-500 max-w-2xl">
            Copy active promo codes and discount vouchers verified by shoppers across popular online stores.
          </p>
